...
//                         <span className="text-[#535359] text-[12px] font-normal leading-[110%] tracking-[-0.24px]">19 May, 01:35pm</span>
//                         <div className="flex">
//                             <div className="flex gap-[5px] items-center pr-3.5">
//                                 <svg
//                                     xmlns="http://www.w3.org/2000/svg"
//                                     viewBox="0 0 8 9"
//                                     className="w-2 h-[9px] text-[#3FAF58]"
//                                     aria-hidden="true"
//                                 >
//                                     <circle cx="4" cy="4.5" r="4" fill="currentColor" />
//                                 </svg>
//                                 <span className="text-[#535359] text-[15px] font-semibold leading-[110%] tracking-[-0.3px]">85%</span>
//                             </div>

//                             <div className="pl-3.5 border-l border-l-[#535359] ">
//                                 <span className="text-[#535359] text-[15px] font-semibold leading-[110%] tracking-[-0.3px]">Acetone: 1.3 ppm</span>
//                             </div>
//                         </div>
//                     </div>
...
//                             <div className="border-l border-l-[#535359] pl-3">
//                                 <span className="text-[#535359] text-[15px] font-semibold leading-[110%] tracking-[-0.3px]">Acetone: 1.3 ppm</span>
//                             </div>
//                         </div>
//                     </div>
...
export default function HistoryTable({ records, activeTab }) {
    ...
    const getRecordValue = (record) => {
        switch(activeTab) {
            case 'sugar':
                return { score: record.Db_Score, additional: `Acetone: ${parseFloat(record.acetone_ppm).toFixed(1)} ppm` };
            case 'liver':
                return { score: record.liver_score, additional: `Ethanol: ${parseFloat(record.ethnol_ppm).toFixed(1)} ppm` };
            case 'respiratory':
                const respiratoryData = JSON.parse(record.respiratory_fvc_json);
                const respiratoryScore = respiratoryData?.["Respiratory Score(%)"]?.["Respiratory Score(%)"];
                return { 
                    score: record.Blow_Score, 
                  additional: `FVC: ${respiratoryScore ? parseFloat(respiratoryScore).toFixed(1) + "%" : "N/A"}`
                };
            case 'gut':
                return { score: record.Gut_Score_per, additional: `H2: ${parseFloat(record.h2_ppm).toFixed(1)} ppm` };
            default:
                return { score: record.Db_Score, additional: `Acetone: ${parseFloat(record.acetone_ppm).toFixed(1)} ppm` };
        }
    };
    ...
    if (!records || records.length === 0) {
        return (
            <div className="w-full flex flex-col gap-5 px-5 pt-[30px] rounded-[20px] bg-white shadow-[0_0_10px_5px_rgba(0,0,0,0.05)]">
                ...
                <div className="max-h-[520px] overflow-y-auto scrollbar-hide">
                    <div className="text-center text-[#535359] py-8">No test records available</div>
                </div>
            </div>
        );
    }
